...
import com.example.demo.finance.loanManagement.services.LoanApplicationApprovalService;
import com.example.demo.finance.loanManagement.services.LoanService;
import com.example.demo.finance.loanManagement.parsistence.models.newApplication;
...
public class LoanApplicationController {
    ...
    @GetMapping("/all")
    @Operation(summary = "Get all loan applications with pagination, filtering, and search")
    public ResponseEntity<Page<LoanApplication>> getAllApplications(
        ...
        Pageable pageable = PageRequest.of(page, size, Sort.by("applicationTime").descending());
        ...
        if (status != null && !status.isEmpty() && !"ALL".equalsIgnoreCase(status)) {
            if (search != null && !search.trim().isEmpty()) {
                ...
                applications = applicationRepo.findByApplicationStatusAndSearch(status, search.trim(), pageable);
            } else {
                // Filter by status only
                applications = applicationRepo.findByApplicationStatus(status, pageable);
            }
        } else if (search != null && !search.trim().isEmpty()) {
            // Search only (across all statuses)
            applications = applicationRepo.findBySearch(search.trim(), pageable);
        } else {
            // No filters - get all
            applications = applicationRepo.findAllByOrderByApplicationTimeDesc(pageable);
        }
