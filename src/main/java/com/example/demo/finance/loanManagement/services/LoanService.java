...
import com.example.demo.finance.banking.services.Dispatcher;
...
public class LoanService {
   ...
   private final RepaymentScheduleEngine repaymentScheduleEngine;
    ...
    public LoanService(ApplicationRepo applicationRepo, TransactionsRepo transactionsRepo, SubscriptionService subscriptionService, CustomerService customerService, ProductService productService, CommunicationService communicationService, InternalChecks internalChecks, LoanAccountRepo loanAccountRepo, Dispatcher dispatcher, Backbone backbone, LoanCalculatorService loanCalculatorService, RepaymentScheduleEngine repaymentScheduleEngine) {
        ...
        this.repaymentScheduleEngine = repaymentScheduleEngine;
    }
...
*/
     ...
     * NOW USES CENTRALIZED LoanCalculatorService and RepaymentScheduleEngine
     */
    public Map<String, Object> createLoanAccountFromApplication(Long applicationId) {
        ...
        // Determine the interest strategy from product
        InterestStrategy strategy = product.getInterestStrategy() != null 
            ? product.getInterestStrategy() 
            : InterestStrategy.FLAT_RATE;
        ...
        log.info("Loan calculation completed: Principal={}, TotalAmount={}, TotalInterest={}", 
        ...
        loanAccount.setAccountBalance((float) calculation.getTotalAmount());
        loanAccount.setCustomerId(customer.getId().toString());
        ...
        log.info("Loan account saved: {}", savedAccount.getLoanref());
        
        // USE REPAYMENT SCHEDULE ENGINE for schedule generation
        try {
            List<LoanRepaymentSchedule> schedules = repaymentScheduleEngine.generateSchedules(
                savedAccount,
                application,
                product,
                calculation
            ...
            log.info("Repayment schedule generated successfully: {} installments", schedules.size());
            ...
            return Map.of(
                ...
                "monthlyPayment", calculation.getMonthlyPayment(),
                "status", savedAccount.getStatus(),
                "schedulesGenerated", schedules.size()
            );
        } catch (Exception e) {
            log.error("Error generating repayment schedule", e);
            // Return account info but flag schedule generation failure
            return Map.of(
                "loanAccountId", savedAccount.getAccountId(),
                "loanReference", savedAccount.getLoanref(),
                "amount", savedAccount.getAmount(),
                "payableAmount", savedAccount.getPayableAmount(),
                "status", savedAccount.getStatus(),
                "error", "Schedule generation failed: " + e.getMessage()
            );
        }
    }
}
