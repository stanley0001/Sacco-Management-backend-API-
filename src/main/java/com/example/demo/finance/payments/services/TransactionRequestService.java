...
import com.example.demo.finance.payments.entities.*;
import com.example.demo.finance.payments.repositories.*;
...
public class TransactionRequestService {
     ...
     * @deprecated Use initiateMpesaDeposit() instead for complete STK push flow
     */
    @Deprecated
    @Transactional
    public TransactionRequest processDepositWithMpesa(Long requestId) {
        ...
        // Check if M-PESA transaction is already linked
        if (request.getMpesaTransactionId() != null) {
            MpesaTransaction mpesaTransaction = mpesaTransactionRepository.findById(request.getMpesaTransactionId())
                .orElse(null);
            if (mpesaTransaction != null) {
                // Already has M-PESA transaction, update status based on transaction status
                if (MpesaTransaction.TransactionStatus.SUCCESS.equals(mpesaTransaction.getStatus())) {
                    request.setStatus(TransactionRequest.RequestStatus.SUCCESS);
                } else if (MpesaTransaction.TransactionStatus.FAILED.equals(mpesaTransaction.getStatus())) {
                    request.setStatus(TransactionRequest.RequestStatus.FAILED);
            ...
            // No M-PESA transaction linked yet, mark as processing
            // Caller should use initiateMpesaDeposit() to create STK push request
            request.setStatus(TransactionRequest.RequestStatus.PROCESSING);
        }
